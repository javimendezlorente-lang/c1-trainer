...
export function isValidAttemptEvent(value: unknown): value is AttemptEvent {
  if (!isRecord(value) || value.eventVersion !== '1.0.0' || value.kind !== 'attempt_submitted' || ![1, 2, 3, 4].includes(value.part as number) || !['multiple_choice_cloze', 'open_cloze', 'word_formation', 'key_word_transformation'].includes(value.type as string)) return false
