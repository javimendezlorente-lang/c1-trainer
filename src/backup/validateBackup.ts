...
export function isValidAttemptEvent(value: unknown): value is AttemptEvent {
  if (!isRecord(value) || value.eventVersion !== '1.0.0' || value.kind !== 'attempt_submitted' || ![1, 2, 3, 4, 5, 6, 7, 8].includes(value.part as number) || !['multiple_choice_cloze', 'open_cloze', 'word_formation', 'key_word_transformation', 'multiple_choice_reading', 'cross_text_multiple_matching', 'gapped_text', 'multiple_matching'].includes(value.type as string)) return false
  ...
  if (typeof value.grade.exerciseId !== 'string' || value.grade.exerciseId !== value.exerciseId || !isFiniteNumber(value.grade.score) || !isFiniteNumber(value.grade.maxScore) || typeof value.grade.complete !== 'boolean' || !Array.isArray(value.grade.results)) return false
  return value.grade.results.every((result) => { if (!isRecord(result) || typeof result.questionId !== 'string' || typeof result.correct !== 'boolean') return false; if (result.answerKind === undefined) return value.part === 1 && (result.selectedOptionId === null || ['A', 'B', 'C', 'D'].includes(result.selectedOptionId as string)) && ['A', 'B', 'C', 'D'].includes(result.correctOptionId as string) && (result.marks === 0 || result.marks === 1); const marks = result.marks; const maxMarks = result.maxMarks; if (!Number.isInteger(marks) || !Number.isInteger(maxMarks) || (maxMarks as number) < 1 || (maxMarks as number) > 2 || (marks as number) < 0 || (marks as number) > (maxMarks as number)) return false; if (!['choice', 'text', 'transformation', 'matching'].includes(result.answerKind as string) || (result.answer !== null && typeof result.answer !== 'string') || typeof result.canonicalAnswer !== 'string' || !Array.isArray(result.acceptedAnswers) || !result.acceptedAnswers.every((item) => typeof item === 'string')) return false; if (result.answerKind === 'choice') return ['A', 'B', 'C', 'D'].includes(result.correctOptionId as string); if (result.answerKind === 'matching') return typeof result.correctTargetId === 'string'; return true })
}
