...
describe('versioned learning backup', () => {
  it('exports only source ledgers and imports an empty backup', async () => {
    ...
    expect(exported.envelope).toMatchObject({ format: 'c1-trainer-backup', version: '1.0.0', app: { databaseVersion: 3 }, attemptEvents: [], reviewEvents: [] })
  ...
  it('round-trips ten attempts and six reviews with equivalent projections and due dates', async () => {
    ...
    await expect(importBackup(backup.json, repository)).resolves.toMatchObject({ attemptEventsAdded: 0, reviewEventsAdded: 0, attemptEventsSkipped: 10, reviewEventsSkipped: 6 })
